...
# for (j in 3:4) {
  print(paste0("bootstrapped dataset b=", b, "/", B, " for model", " j=",j,"/",length(model_names_list)))
  ...
  if (model_type == "XGB") {
    ...
    mlr_model_name = str_remove_all(target_model_name, "_weightByDrive|_weightByEpoch|_randomlyDrawOnePlayPerGroup")
    fit_mlr_func = get(paste0("fit_", mlr_model_name))
  }
  model_is_weightedByEpoch = str_detect(target_model_name, "weightByEpoch")
  model_is_weightedByDrive = str_detect(target_model_name, "weightByDrive")
  model_is_randomlyDrawOnePlayPerGroup = str_detect(target_model_name, "randomlyDrawOnePlayPerGroup")
  
  if (train_test) {
    dataset = train_set
    filename_prefix = "trainedModelForTesting_"
  } else {
    dataset = data_full
  ...
  if (b == 0) { ### use original training dataset
    ...
    set.seed(22+b*3788)
    if (model_is_weightedByDrive | model_is_weightedByEpoch | model_is_randomlyDrawOnePlayPerGroup) {
  ...
  if (!model_is_randomlyDrawOnePlayPerGroup) {
    if (model_type == "XGB") {
      fit <- train_xgb(
        ...
        weight_by_epoch=model_is_weightedByEpoch, weight_by_drive=model_is_weightedByDrive,
