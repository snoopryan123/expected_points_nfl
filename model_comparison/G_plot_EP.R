...
### make sure to first train and save the models
# PASS = 1 #FIXME
PASS = 2 #FIXME
# PASS = 3 #FIXME
...
if (PASS == 1) {
  MODEL_TYPE = "MLR"
  model_name = "mlr_epochEP_yurko_plus_tq_weightByEpoch"
  mlr_model_name = str_remove_all(model_name, "_weightByDrive|_weightByEpoch")
  model_fit = readRDS(paste0("fitted_models/trainedFullModel_",model_name,"_b0.rds"))
} else if (PASS == 2) {
  MODEL_TYPE = "XGB"
  model_name = xgb_C_epochEP_oq2xdq2x_1_weightByEpoch_model_name
  model_fit = readRDS(paste0("fitted_models/trainedFullModel_",model_name,"_b0.rds"))
...
plot_varyingTime <- function(model_fit, model_name, half_=1) {
  ...
  for (j in length(time_breaks):1) {
    plot_set = bind_rows(
      ...
      tibble(
        ...
        qbqot = 0, oqot = 0, dqdt_againstPass = 0, dqdt_againstRun = 0,
        qbqdt = 0, oqdt = 0, dqot_againstPass = 0, dqot_againstRun = 0,
  ...
  plot
  ggsave(paste0("plotting/plot_model_",model_name,"_time.png"), plot,width=9,height=7)
  # return(plot)
}
...
plot_varyingTimeByDown <- function(model_fit, model_name, half_=1) {
  ...
  plot
  ggsave(paste0("plotting/plot_model_",model_name,"_timeByDown.png"), plot,width=11,height=9)
  # return(plot)
}
...
plot_varyingTQ <- function(model_fit, model_name, colname, N=7, keepFewSpreads=FALSE, noTitle=FALSE) {
  ...
  if (!spread_tq) {
    ...
    for (j in 1:N) {
      plot_set = bind_rows(
        ...
        tibble(
          yardline_100 = 1:99,
          ydstogo=c(1:9, rep(10,90)), 
          qbqot = ifelse(colname == "qbqot", tq_breaks[j], 0),
          oqot = ifelse(colname == "oqot", tq_breaks[j], 0),
          qbqdt = ifelse(colname == "qbqdt", tq_breaks[j], 0),
          oqdt = ifelse(colname == "oqdt", tq_breaks[j], 0),
          dqdt_againstPass = ifelse(colname == "dqdt_againstPass", tq_breaks[j], 0),
          dqot_againstPass = ifelse(colname == "dqot_againstPass", tq_breaks[j], 0),
          dqdt_againstRun = ifelse(colname == "dqdt_againstRun", tq_breaks[j], 0),
          dqot_againstRun = ifelse(colname == "dqot_againstRun", tq_breaks[j], 0),
          down=1, down1=1, down2=0, down3=0, down4=0, down_combined34 = ifelse(down==3|down==4,34,down),
    ...
    legend_title = case_when(
      colname == "qbqot" ~ " offensive\n team's\n quartberack\n quality",
      colname == "oqot" ~ " offensive\n team's\n remaining\n offensive\n quality",
      colname == "qbqdt" ~ " defensive\n team's\n quartberack\n quality",
      colname == "oqdt" ~ " defensive\n team's\n remaining\n offensive\n quality",
      colname == "dqdt_againstPass" ~ " defensive\n team's\n defensive\n quality\n against\n the pass",
      colname == "dqot_againstPass" ~ " offensive\n team's\n defensive\n quality\n against\n the pass",
      colname == "dqdt_againstRun" ~ " defensive\n team's\n defensive\n quality\n against\n the run",
      colname == "dqot_againstRun" ~ " offensive\n team's\n defensive\n quality\n against\n the run",
      TRUE ~ "FIXME"
    )
  } else {
...
plot_varyingDown <- function(model_fit, model_name) {
  ...
  plot = pred_ep %>%
    ...
    scale_colour_manual(values = my_palette)
  plot
  ggsave(paste0("plotting/plot_model_",model_name,"_down.png"), plot,width=9,height=7)
  # return(plot)
}
...
plot_varyingScoreDiff <- function(model_fit, model_name) {
  ...
  plot
  ggsave(paste0("plotting/plot_model_",model_name,"_scoreDiff.png"), plot,width=9,height=7)
  # return(plot)
}
...
make_plots_batch <- function(model_name, model_fit) {
  if (!spread_tq) {
    ...
    # p1 = plot_varyingTQ(model_fit, model_name, "qbqot")
    # p2 = plot_varyingTQ(model_fit, model_name, "oqot")
    # p3 = plot_varyingTQ(model_fit, model_name, "dq_dt_0_total_sum")
    # p4 = plot_varyingTQ(model_fit, model_name, "qbqdt")
    # p5 = plot_varyingTQ(model_fit, model_name, "oqdt")
    # p6 = plot_varyingTQ(model_fit, model_name, "dq_ot_0_sum")
    p1 = plot_varyingTQ(model_fit, model_name, "qbqot", noTitle=T)
    p2 = plot_varyingTQ(model_fit, model_name, "oqot", noTitle=T)
    p3 = plot_varyingTQ(model_fit, model_name, "dqdt_againstPass", noTitle=T)
    p4 = plot_varyingTQ(model_fit, model_name, "dqdt_againstRun", noTitle=T)
    p5 = plot_varyingTQ(model_fit, model_name, "qbqdt", noTitle=T)
    p6 = plot_varyingTQ(model_fit, model_name, "oqdt", noTitle=T)
    p7 = plot_varyingTQ(model_fit, model_name, "dqot_againstPass", noTitle=T)
    p8 = plot_varyingTQ(model_fit, model_name, "dqot_againstRun", noTitle=T)
    
    p18 = plot_grid(p1,p2,p3,p4,p5,p6,p7,p8, nrow=2)
    save_plot(paste0("plotting/plot_model_",model_name,"_8TQ.png"), p18, base_width=30, base_height=12)
    
    # if (no_dq) {
    #   p14 = plot_grid(p1,p2,p4,p5, nrow=2 )
    #   save_plot(paste0("plotting/plot_model_",model_name,"_4OQ.png"), p14, base_width=16, base_height=12)
    # } else {
    #   p18 = plot_grid(p1,p2,p3,p4,p5,p6,p7,p8, nrow=2)
    #   # p18 = plot_grid(p1,p2,p3,p4,p5,p6,p7,p8, nrow=2,
    ...
    #   # save_plot(paste0("plotting/plot_model_",model_name,"_6TQ.png"), p16, base_width=24, base_height=12)
    # }
    
    # save_plot(paste0("plotting/plot_model_",model_name,"_diagonalTQ.png"), pd, base_width=8, base_height=6)
    ## return(p1234)
    # p1 = plot_varyingTQ(model_fit, model_name, "oq_ot_0_total_sum")
    # p2 = plot_varyingTQ(model_fit, model_name, "dq_dt_0_total_sum")
    # p12 = plot_grid(p1,p2)
    # save_plot(paste0("plotting/plot_model_",model_name,"_2TQ.png"), p12, base_width=14, base_height=5)
    # ## return(p12)
  } else {
    p1 = plot_varyingTQ(model_fit, model_name, "")
    ggsave(paste0("plotting/plot_model_",model_name,"_sTQ.png"), p1, width=9, height=7)
    if (MODEL_TYPE=="XGB" & str_detect(model_name, "xgb_C")) {
      p2 = plot_varyingTQ(model_fit, model_name, "", keepFewSpreads=TRUE) 
      ggsave(paste0("plotting/plot_model_",model_name,"_sTQoverfitting.png"), p2, width=9, height=7)
    }
  }
}
