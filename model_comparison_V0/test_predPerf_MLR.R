...
get_loss <- function(preds_MLR, test_set) {
  preds_MLR %>% 
    group_by(model) %>%
    summarise(
...
results_MLR = get_loss(preds_MLR, test_set)
# print(data.frame(results_MLR))

idxs_to_keep = (test_set %>% mutate(ii = 1:n()) %>% filter(down!=4 & half_seconds_remaining > 3*60))$ii
test_set_truncated = test_set[idxs_to_keep,]
preds_MLR_truncated = preds_MLR %>% group_by(model) %>% filter(row_number() %in% idxs_to_keep)

results_MLR_truncated = get_loss(preds_MLR_truncated, test_set_truncated)

print(data.frame(results_MLR))
print(data.frame(results_MLR_truncated))

write_csv(results_MLR, "test_results_predPerf_MLR.csv")
write_csv(results_MLR_truncated, "test_results_tuncated_predPerf_MLR.csv")
